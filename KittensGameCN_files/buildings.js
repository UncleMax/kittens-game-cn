dojo.declare("classes.managers.BuildingsManager", com.nuclearunicorn.core.TabManager, {
	...
	//----------------------------------- Resource production ----------------------------------------
	{
		...
		action: function(self, game){
			...
			if (game.workshop.get("deepMining").researched){
				//fun but ugly hack
				self.effects["coalPerTickBase"] = 0.2;
	...
	},
	{
		...
		action: function(self, game){
			...
			self.effects["ironPerTick"] = 0.2 * smelterRatio * autoProdRatio;
			...
			if (wood.value > self.on * -self.effects["woodPerTick"] &&
				...
				if (game.workshop.get("goldOre").researched){
					self.effects["goldPerTick"] = 0.01 * autoProdRatio;
	...
	},
	{
		...
		action: function(self, game){
			...
			self.effects["coalRatioGlobal"] = -0.6 + coalRatio;
...

	{
		...
		action: function(self, game){
			...
			self.effects["woodRatio"] = 0.2 + 0.2*ratio;
		},
		flavor: "最好的日志分析工具"
	},
	{
		...
		action: function(self, game){
			...
			self.effects["oilPerTickBase"] = 0.2 + 0.2*ratio;
		},
	...
	//----------------------------------- Other ----------------------------------------
	{
		name: "workshop",
		label: "车间",
		description: "提供大量的升级。改进工艺效果百分之20",
		unlocked: false,
		prices: [
			{ name : "wood", val: 100 },
		...
		effects: {
			"craftRatio" : 1	//6% for craft output
		},
		priceRatio: 1.10,
		handler: function(btn){
	...
	//-------------------------- Culture -------------------------------
	{
		...
		effects: {
			"unhappinessRatio" : -0.3,
			"culturePerTickBase" : 0.2,
			"cultureMax" : 50
		},
		priceRatio: 1.0,
		ignorePriceCheck: true,
