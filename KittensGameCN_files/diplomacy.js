...
dojo.declare("classes.managers.DiplomacyManager", null, {
	...
	races: [{
		name: "lizards",
		title: "蜥蜴",
		attitude: "友善",	//neutral, friendly, aggressive
	...
	update: function(){
		if (!this.hasUnlockedRaces()){
			...
			this.game.msg("一个使者" + race.title + " 来到你的村子", "notice");
		}
	}
...
dojo.declare("com.nuclearunicorn.game.ui.TradeButton", com.nuclearunicorn.game.ui.ButtonModern, {
	...
	tradeInternal: function(suppressMessages){
		...
		if (race.attitude == "hostile" && this.game.rand(100) - standingRatio >= race.standing * 100){	//the less you roll the better
			if (!suppressMessages){
				this.game.msg( race.title + " 无缘无故的恨你");
			}
			return;
		}
		...
		if (race.attitude == "friendly" && this.game.rand(100) - standingRatio/2 <= race.standing * 100){	//confusing part, low standing is ok for friendly races
			if (!suppressMessages){
				this.game.msg( race.title + " 认为你的猫是可爱的");
			}
	...
	printYieldOutput: function(yieldResTotal){
		for (var res in yieldResTotal) {
			if (res != "blueprint" && res != "titanium"){
				this.game.msg("你获得了 " + this.game.getDisplayValueExt(yieldResTotal[res]) + " " + res);
			} else {
				this.game.msg("你获得了 " + this.game.getDisplayValueExt(yieldResTotal[res]) + " " + res + "!", "notice");
			}
		}
	},
...
dojo.declare("com.nuclearunicorn.game.ui.tab.Diplomacy", com.nuclearunicorn.game.ui.tab, {
	...
	render: function(tabContainer){
		...
		for (var i = 0; i< races.length; i++){
			...
			dojo.create("div", { innerHTML: "态度: " + race.attitude, style: {
				marginBottom: "5px"
			} }, content);

			var buys = race.buys[0];
			// console.log("show :", buys);
			dojo.create("div", {
				innerHTML: "<span style='color: #01A9DB'>购买: </span>" + this.game.getResourceTitle(buys.name) + " (" + buys.val + ")"
			}, content);
			...
			for (var j =0; j< race.sells.length; j++){
				//if (race.sells[j].chance == 100){
					...
					var prefix = ( j == 0) ? "<span style='color: green'>出售: </span>" : "";
					var div = dojo.create("div", { innerHTML: prefix + this.game.getResourceTitle(s.name) + " (" + min.toFixed() + " - " + max.toFixed() + ")"}, content);
			...
			var tradeBtn = new com.nuclearunicorn.game.ui.TradeButton({
				name: "派出商队",
				description: "交易你的一些东西来提供资源. 随着季节变化，价格可能会有所不同.\n你也有一个小的机会获得稀有资源.",
				prices: tradePrices,
				race: race
			}, this.game);
		...
		var exploreBtn = new com.nuclearunicorn.game.ui.ButtonModern({
			...
			handler: function(btn){
				...
				if (race){
					btn.game.msg("你找到了一个新的文明!", "notice");
				} else {
					btn.game.msg("你的探险家失败了，没有找到任何东西.");
